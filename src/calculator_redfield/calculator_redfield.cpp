	...
	calculator_redfield::calculator_redfield(
		...
		(storage<asymptoticLF_complexv>& imfun,
				...
				if(!rates.IsAlloc())
				{
					...
					for(int iosc = 0; iosc<numBosc; iosc ++)
					{
						...
						if(ia != ib)
						{
							...
							// "from" stare gets reorganized
							omegaij += reorganizations.data2D[ib][ib];
			...
			storage<complexv> calculator_redfield::AddLifetimeDephasings()
			{
				// here I create redfield rates and
				// dephasing constants from mfun
				// notice that this is the coherent part, the system evolves with respect to
				// ground state so no reorganizations added
			...
			storage<complexv> calculator_redfield::GetRelaxationSuperoperatorM(int block)
			{
				...
				// special extension goes separately
				if (flagLindblad == 1) return GetRelaxationSuperoperatorLindblad1(block);


				// getting back to normal Redfield
				// adding part that if rightmost density matrix is in population state
				// the density matrix relaxes to this state and the energies shift by reorganiztions
				...
				for(int ib1 = 0; ib1<numR; ib1++)
				{
					complexv value = 0.0;
					int population = 0;
					if(ia1+shL==ib1+shR) population = 1;
					...
					for(int iosc = 0; iosc<numBosc; iosc++)
					{
						...
						if(ib == ib1)
						{
							for(int ic=0; ic<numL; ic++)
							{
								...
								// reorganizations of state 2 with respect to state 1: assuming all shifts are positive
								if(population)
								omegaij += -reorganizations.data2D[ia1+shL][ia1+shL]
								- (reorganizations.data2D[ia1+shL][ia1+shL]-2*sqrt(reorganizations.data2D[ic+shL][ic+shL]*reorganizations.data2D[ia1+shL][ia1+shL]));
						...
						if(ia == ia1)
						{
							for(int ic=0; ic<numR; ic++)
							{
								...
								// reorganizations of state 2 with respect to state 1: assuming all shifts are positive
								if(population)
								omegaij += -reorganizations.data2D[ib1+shR][ib1+shR]
								- (reorganizations.data2D[ib1+shR][ib1+shR]-2*sqrt(reorganizations.data2D[ic+shR][ic+shR]*reorganizations.data2D[ib1+shR][ib1+shR]));
						...
						if(true)
						{
							...
							if(population)
							omegaij += -reorganizations.data2D[ib1+shR][ib1+shR]
							- (reorganizations.data2D[ib1+shR][ib1+shR]-2*sqrt(reorganizations.data2D[ib+shR][ib+shR]*reorganizations.data2D[ib1+shR][ib1+shR]));
						...
						if(true)
						{
							...
							if(population)
							omegaij += -reorganizations.data2D[ia1+shL][ia1+shL]
							- (reorganizations.data2D[ia1+shL][ia1+shL]-2*sqrt(reorganizations.data2D[ia+shL][ia+shL]*reorganizations.data2D[ia1+shL][ia1+shL]));
				...
				for(int it=0;it<numT;it++)
				{
					...
					for(int ib1=0;ib1<numR;ib1++)
					{
						...
						for(int ic=0;ic<numL;ic++)
						{
							...
							// reorganizations of state 2 with respect to state 1: assuming all shifts are positive
							if(population)
							omegat -= -reorganizations.data2D[ia1+shL][ia1+shL]
							- (reorganizations.data2D[ia1+shL][ia1+shL]-2*sqrt(reorganizations.data2D[ic+shL][ic+shL]*reorganizations.data2D[ia1+shL][ia1+shL]));
						...
						for(int id=0;id<numR;id++)
						{
							...
							// reorganizations of state 2 with respect to state 1: assuming all shifts are positive
							if(population)
							omegat += -reorganizations.data2D[ib1+shR][ib1+shR]
							- (reorganizations.data2D[ib1+shR][ib1+shR]-2*sqrt(reorganizations.data2D[id+shR][id+shR]*reorganizations.data2D[ib1+shR][ib1+shR]));
							for(int io=0;io<numosc;io++)
